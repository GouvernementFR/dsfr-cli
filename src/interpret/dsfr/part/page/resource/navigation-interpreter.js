import { log, normalizeId } from '@gouvfr/dsfr-forge';
...
class NavigationInterpreter {
  ...
  _interpretItems (wrapper) {
    ...
    const items = this._state.resolveItems(wrapper.path, wrapper.rank);
    if (items && items.length) return items;
  }
  ...
  _interpretSidemenu () {
    ...
    const items = this._state.resolveItems(sidemenu.match, sidemenu.rank, sidemenu.sort);

    const isCollapsible = sidemenu.isCollapsible !== false;

    if (!isNaN(sidemenu.depth) && sidemenu.depth >= 2) {
      items.forEach(item => {
        const items2 = this._state.resolveItems(item.path, sidemenu.rank, sidemenu.sort);
        if (items2.length) {
          item.type = 'menu';
          ...
          if (sidemenu.depth >= 3) {
            items2.forEach(item => {
              const items3 = this._state.resolveItems(item.path, sidemenu.rank, sidemenu.sort);
              if (items3.length) {
                item.type = 'menu';
                item.isCollapsible = isCollapsible;
                item.items = items3;
